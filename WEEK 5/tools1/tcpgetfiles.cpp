/*
 * 程序名：tcpgetfiles.cpp，此程序用于演示socket通讯的客户端。
 * 作者：moonhoro
 */
...
struct st_arg
{
  ...
  char clientpath[301]; // 服务端文件存放的根目录。
  bool andchild;        // 是否下载clientpath目录下各级子目录的文件，true-是；false-否。
  char matchname[301];  // 待下载文件名的匹配规则，如"*.TXT,*.XML"。
  char srvpath[301];    // 客户端文件存放的根目录。
  int timetvl;          // 扫描服务端文件的时间间隔，单位：秒。
...
{
  printf("\n");
  printf("Using:/root/root/project/tools1/bin/tcpgetfiles logfilename xmlbuffer\n\n");

  printf("Sample:/root/project/tools1/bin/procctl 20 /root/project/tools1/bin/tcpgetfiles /root/log/idc/tcpgetfiles_surfdata.log \"<ip>8.130.41.197</ip><port>5005</port><ptype>1</ptype><srvpath>/root/tmp/tcp/surfdata2</srvpath><andchild>true</andchild><matchname>*.XML,*.CSV,*.JSON</matchname><clientpath>/root/tmp/tcp/surfdata3</clientpath><timetvl>10</timetvl><timeout>50</timeout><pname>tcpgetfiles_surfdata</pname>\"\n");
  printf("       /root/project/tools1/bin/procctl 20 /root/project/tools1/bin/tcpgetfiles /root/log/idc/tcpgetfiles_surfdata.log \"<ip>8.130.41.197</ip><port>5005</port><ptype>2</ptype><srvpath>/root/tmp/tcp/surfdata2</srvpath><srvpathbak>/root/tmp/tcp/surfdata2bak</srvpathbak><andchild>true</andchild><matchname>*.XML,*.CSV,*.JSON</matchname><clientpath>/root/tmp/tcp/surfdata3</clientpath><timetvl>10</timetvl><timeout>50</timeout><pname>tcpgetfiles_surfdata</pname>\"\n\n\n");
