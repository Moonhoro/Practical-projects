/*
 *程序名:xmltodb.cpp 用于把xml文件入库到MySQL
 *作者：moonhoro
 */

#include "/root/project/tools1/c/_tools.h"
/*
// 表的列(字段)信息的结构体。
struct st_columns
{
...
void preparesql();
// 在处理xml之前，如果stxmltotable不为空，就执行它
bool execsql();
...
bool _xmltodb()
{
  ...
  while (true)
  {
    ...
    while (true)
    {
      ...
      // 打开x1文件错误，这种错误一般不会发生，如果真发生了，程序将退出
      if (iret == 3)
      {
        logfile.WriteEx("failed,打开xml文件失败。n");
        return false;
      }
      //  数据库错误，函数返回，程序将退出。
      if (iret == 4)
      ...
      // 在处理xm1文件之前，如果执行stxmltotab1e.execsql失败，函数返回，程序将退出
      if (iret == 6)
      {
