/*
 *  程序名：deletetable.cpp，本程序是数据中心的公共功能模块，定期删除数据库里的数据
 *  作者：moonhoro
 */
#include "_tools.h"
...
connection conn1; // 本地数据库连接。
connection conn2; // 远程数据库连接。
...
void _help(char *argv[])
{
    printf("Using:/root/project/tools1/bin/deletetable logfilename xmlbuffer\n\n");

    printf("Sample:/root/project/tools1/bin/procctl 3600 /root/project/tools1/bin/deletetable /root/log/idc/deletetable_ZHOBTMIND1.log \"<connstr>127.0.0.1,root,Ggh.20020902,mysql,3306</connstr><tname>T_ZHOBTMIND1</tname><keycol>keyid</keycol><where>where ddatetime<timestampadd(minute,-120,now())</where><starttime>01,02,03,04,05,13</starttime><timeout>120</timeout><pname>deletetable_ZHOBTMIND1</pname>\"\n\n");
